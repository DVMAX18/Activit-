local Config = {
    EnemyHPScaling = 3, -- Par joueur, multipliez ennemi HP par 1 + ennemihpscaling * 0,1
    BaseHUDOffset = 1, -- Correspond directement au décalage HUD dans les options
    PVP = 1, -- 1 = Off, 2 = On
    MultiplayerBossSplash = 2, -- 1 = Off, 2 = On
    GhostDeathMode = 4, -- 1 = Off, 2 = Les joueurs deviennent des fantômes à la mort plutôt que sur la fin du jeu, 3 = les fantômes peuvent être ravivés en faisant un sacrifice dans la salle de sacrifice
    PlayerTintMode = 2, -- 1 = Off, Les joueurs utiliseront tous des couleurs normales, 2 = les joueurs utiliseront des couleurs teintées tout en maintenant le bouton de la carte, 3 = les joueurs seront toujours teintés
    HUDOutlineMode = 2, -- 1 = Off, 2 = Contour lors de la maintenance du bouton de carte, 3 = toujours décrit
    PlayerTextMode = 1, -- 1 = Off, 2 = Sur tout en maintenant le bouton de la carte, 3 = toujours sur
    HUDTextMode = 1, -- 1 = Off, 2 = Sur tout en maintenant le bouton de la carte, 3 = toujours sur
    ...
    ExtraLifeHUDOffsets = { -- De l'article actif
        nil,
        Vector(64, 0),
        Vector(-11, -27),
        Vector(66, 0)
    ...
    ColorFadeFrames = 15, -- Cadres qu'il faut pour que la couleur apparaisse / disparaît lorsque l'onglet est enfoncé / libéré
    PlayerColors = {
...
local function random(min, max, rng) -- Remplements Math.Random ()
    rng = rng or RandomRNG
    if min ~= nil and max ~= nil then -- Min et Max passé, entier [min, max]
        return math.floor(rng:RandomFloat() * (max - min + 1) + min)
    elseif min ~= nil then -- Seulement min passe, entier [0, min]
    ...
    for variant, playersData in pairs(ExpectingPerPlayer) do
        for pind, familiarData in pairs(playersData) do
            ...
            for _, fam in ipairs(familiars) do
                ...
                if data.TrueCoopPlayerListIndex == pind then
                    familiarData.Found = familiarData.Found + 1
                elseif not data.TrueCoopPlayerListIndex then
                    fam:Remove()
                end
            end

            if familiarData.Found < familiarData.Count then -- Spawn plus familiers
            ...
            elseif familiarData.Found > familiarData.Count then -- Supprimer les familiers
                local numToRemove = familiarData.Found - familiarData.Count
                local numRemoved = 0
                local familiars = Isaac.FindByType(EntityType.ENTITY_FAMILIAR, variant, -1, false, false)
                for _, fam in ipairs(familiars) do
                    if fam:GetData().TrueCoopPlayerListIndex == pind then
                        fam:Remove()
...
local abpp = require("ab++")
function EntityPlayer.inst:CheckFamiliar(variant, count, rng, calcBoxOfFriends)
...
local oldGetPlayerType = EntityPlayer.inst.GetPlayerType
function EntityPlayer.inst:GetPlayerType(noHijack)
...
local oldGetName = EntityPlayer.inst.GetName
function EntityPlayer.inst:GetName()
    ...
    if not data.TrueCoop.JustCalledUpdate then
        ...
        if data.TrueCoop.CoopPlayer then
            ...
            if data.TrueCoop.JustSpawned then
                local stolenMax, stolenHearts, stolenSoul = player:GetMaxHearts(), player:GetHearts(), player:GetSoulHearts()
                local stolenBlack = {}
                for i = 1, 24 do
                    if player:IsBlackHeart(i) then
                        stolenBlack[i] = true
                        stolenBlack[i + 1] = true
                    end
                end

                local oriPlayer = players[1]

                for i = 1, stolenMax + stolenSoul do
                    if i <= stolenMax then
                        oriPlayer:AddMaxHearts(1)
                    else
                        if stolenBlack[i] then
                            oriPlayer:AddBlackHearts(1)
                        else
                            oriPlayer:AddSoulHearts(1)
                        end
                    end
                end
        ...
        elseif data.TrueCoop.JustSpawned then -- Le joueur d'origine vient de donner naissance
            local pind = data.TrueCoop.PlayerListIndex
...
local function RenderPlayerHUD(pind, player, tintColor, renderOutlines, renderMain, isMapActive)
    ...
    if renderMain and goldHearts > 0 and not (game:GetLevel():GetCurses() & LevelCurse.CURSE_OF_THE_UNKNOWN == LevelCurse.CURSE_OF_THE_UNKNOWN) then
        local hudOffset
        ...
        local lastHeart = player:GetMaxHearts() + player:GetSoulHearts()
...
local function PlayTextStreak(text, extratext, extratextOffset, extratextScaleMulti)
    ...
    if not StreakSprites[index] then -- Ce système charge autant de sprites qu'il doit jouer à la fois
    ...
    if collider:ToPlayer() then
        ...
        if pickup.Variant == PickupVariant.PICKUP_HEART and not pickup:IsDead() then
            ...
            or pickup.SubType == HeartSubType.HEART_DOUBLEPACK then
                if pickup:IsShopItem() and (player:GetNumCoins() < pickup.Price or player:GetHearts() == player:GetMaxHearts() or (pdata.TrueCoopPickupDelay and pdata.TrueCoopPickupDelay > 0)) then
                    return true
                end

                if player:GetHearts() ~= player:GetMaxHearts() and not pdata.TrueCoopCollected and not (pdata.TrueCoopPickupDelay and pdata.TrueCoopPickupDelay > 0) then
...
bossSprite:Load("gfx/truecoop/versusscreen/versusscreen_coop.anm2")

local function TrueCoopBossRender()
    local playing = (bossSprite:IsPlaying("2P") or bossSprite:IsPlaying("3P") or bossSprite:IsPlaying("4P"))
    ...
    if game:IsPaused() and not ended and playing then
        ...
        bossSprite:Render(GetScreenCenterPosition(), zeroVector, zeroVector)
    elseif playing then
        bossSprite:Stop()
    end
end
...
mod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
    ...
    if room:GetType() == RoomType.ROOM_BOSS and not room:IsClear() and Config.MultiplayerBossSplash == 2 then
        ...
        if numPlayers > 1 then
            for layer = 11, 12 - numPlayers, -1 do
                ...
                if name then
                    ...
                    if byName then
                        ...
                        if portrait then
                            bossSprite:ReplaceSpritesheet(layer, portrait)
                        end
                    end
                end
            end
            ...
            if numPlayers == 2 then
                bossSprite:ReplaceSpritesheet(2, spot)
            else
                bossSprite:ReplaceSpritesheet(3, spot)
            end

            bossSprite:LoadGraphics()
            bossSprite:Play(tostring(numPlayers) .. "P", true)
        end
    end
end)
